    ...
    with torch.no_grad():
        for i, batch_data in enumerate(data_loader):
            ...
            img_emb, cap_emb, cap_len = model.forward_emb(
                part_emb, captions, lengths, sentences)
...
def i2t(images, captions, caplens, model_index_image, model_index_text, sims, caption_idx_li, devocab, npts=None,
        return_ranks=False):
    """
...
def t2i(images, captions, caplens, model_index_image, model_index_text, sims, caption_idx_li, devocab, npts=None,
        return_ranks=False):
    """
    ...
    with torch.no_grad():
        for i, val_data in enumerate(data_loader):
            ...
            if torch.cuda.is_available():
                shapes, semantic_labels = shapes.to(dtype=torch.float).cuda(), semantic_labels.to(
...
def validate(opt, val_loader, model, devocab, epoch, return_ranks=False):
    ...
    if epoch >= opt.stage_1_epoch:
        ...
        # caption retrieval
        i2t_res = i2t(
            img_embs, cap_embs, cap_lens, model_index_image, model_index_text, sims, caption_idx_li, devocab,
