...
def main():
    ...
    train_loader_shuffle = dataloader_module.get_loaders_shuffle(vocab, opt.batch_size, opt.workers, opt)

    # Construct the model
    model = model_module.SCA3D(opt)
    ...
    for epoch in range(start_epoch, opt.num_epochs):
        ...
        # train for one epoch
        train(opt, train_loader, train_loader_shuffle, model, epoch)
        scheduler.step()
...
def train(opt, train_loader, train_loader_shuffle, model, epoch):
    ...
    for (i, train_data), (j, train_data_shuffle) in zip(track(enumerate(train_loader), total=len(train_loader)),
        ...
        model.train_emb(train_data_shuffle, epoch=epoch)
